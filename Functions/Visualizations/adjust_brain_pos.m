...
if strcmp(params.ctx, 'std')
    switch params.orientation % If volume is not in transverse orientation, put it there
        case 's'
            Nln = size(Lnodes, 1);
...
if strcmp(params.view, 'lat')  ||  strcmp(params.view, 'med')
    cmL = mean(Lnodes, 1); % rotate right hemi around and move to position for visualization
    cmR = mean(Rnodes, 1);
    rm = rotation_matrix('z', pi);
    
    switch params.view % Rotate
        case 'lat'
            Rnodes = (Rnodes - (repmat(cmR, size(Rnodes, 1), 1))) * rm + (repmat(cmR, size(Rnodes, 1), 1));
        case 'med'
            Lnodes = (Lnodes - (repmat(cmL, size(Lnodes, 1), 1))) * rm + (repmat(cmL, size(Lnodes, 1), 1));
    end
